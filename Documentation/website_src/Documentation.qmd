...
<!--TODO-->
install vitis ai pytorch docker container
Download prebuilt cpu docker image 
 docker pull xilinx/vitis-ai-cpu:latest
Build cpu docker image
cd setup/docker
 ./docker_build_cpu.sh
Run docker image:  (May need to update run script)
...
<!--TODO-->
(Optional) setup sd boot image
Download prebuilt image from Xilinx and burn image on to SD card
insert sd card into FPGA board and set to SD boot mode
...
![PuTTy Connection Settings](res/putty2.png)
...
Alternatively, a static IP can be given to the board by running `ifconfig eth0 <IPAdress>` on the board. This may interfere with the DHCP running on the VCU network and might not work as intended. This method is not recommended.
...
Note: `-X` enables X11 Forwarding, which is not necessary.
